...
async function getStatsBulanIni() {
    ...
    // Ambil rentang awal dan akhir bulan ini (Zona Waktu Jakarta)
    const now = new Date();
    const firstDay = new Date(now.getFullYear(), now.getMonth(), 1).toISOString().split('T')[0];
    const lastDay = new Date(now.getFullYear(), now.getMonth() + 1, 0).toISOString().split('T')[0];
    const { data, error } = await supabase.from('surveilans_harian').select('*').eq('user_id', user.id).gte('tanggal', firstDay).lte('tanggal', lastDay);
    ...
    return {
        ...
        totalTindakan: data.reduce((acc, curr)=>acc + (Number(curr.uc) || 0) + (Number(curr.cvl) || 0) + (Number(curr.ivl) || 0) + (Number(curr.ett) || 0), 0),
        potensiHais: data.reduce((acc, curr)=>acc + (Number(curr.vap) || 0) + (Number(curr.ido) || 0) + (Number(curr.isk) || 0) + (Number(curr.iad) || 0), 0)
    };
}
}),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b8bf89c1._.js.map